...
--3.UPDATE
--1. Всех записей
UPDATE book SET name = 'К западу от границы , к югу от солнца'
...
SELECT * FROM reader WHERE id_reader = '3'
...
--1
SELECT first_name FROM author GROUP BY first_name HAVING MAX(id_author) > 1
--2
...
LEFT JOIN book ON (author.id_author = book.id_book) 
-- 3-ю по другому:
LEFT JOIN reader ON (reader.id_reader = author.id_author) 
WHERE  author.id_author = 6 AND gender = 'М' AND book.id_author = 2  --условие для поля 1-ой таблицы
